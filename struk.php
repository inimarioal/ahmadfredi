...
// $input_code = $conn->query("SELECT from `reservation_list` where code = '{$code}'");
if (empty($code)) {
            ...
            <form action="" id="filter-report">
                <div class="row align-items-end">
                    <div class="form-group col-md-3">
                        <small class="text-muted mx-2">Reservation Code</small>
                        <input type="text" name="code" value="<?= $code; ?>" class="form-control form-control-sm rounded-0" required>
                    </div>
                    <div class="form-group col-md-3">
        ...
        <div class="container-fluid" id="outprint">
			...
			<table class="table table-hover table-striped table-bordered">
				...
				<tbody>
					<?php 
						$i = 1;
						$qry = $conn->query("SELECT r.*,rr.name as room_name, rr.type as room_type from `reservation_list` r inner join `room_list` rr on r.room_id = rr.id where ( r.code = '{$code}') order by r.`status` asc, unix_timestamp(r.`date_created`) desc ");
...
<script>
	$(document).ready(function(){
		$('#filter-report').submit(function(e){
            e.preventDefault();
            location.href = './?page=struk&'+$(this).serialize()
        })
